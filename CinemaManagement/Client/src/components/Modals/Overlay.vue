...
<template>
  <div class="absolute inset-0 fixed bg-black bg-opacity-50">
    <div class="flex items-start justify-center min-h-screen mt-24 text-center" role="dialog" aria-modal="true">
      <div class="bg-white relative bottom-50 my-40 mx-1/3 text-black rounded-lg text-center shadow-xl p-10 w-full h-full">
        ...
        <slot/>
        <button v-if="handleSubmit" type="submit" @click.prevent="handleSubmit"
                class="css-button-sliding-to-bottom--red">
          Submit
        </button>
      </div>
    </div>
